...
uint32_t *ret = new uint32_t[100]; ;
    ...
    // return ret;   //USBTRACE2("Total:", totalSize);
}
...
uint32_t ReportDescParserBase::PrintByteValue(uint8_t data) {
   // E_Notify(PSTR("("), 0x80);
...
uint32_t ReportDescParserBase::ParseItem(uint8_t **pp, uint16_t *pcntdn) {
...
uint32_t ReportDescParser2::ParseItem(uint8_t **pp, uint16_t *pcntdn) {
        //uint8_t	ret = enErrorSuccess;
	uint8_t* dataIn = new uint8_t[100] ;
	uint32_t test[100];
						uint32_t var=100;
        switch (itemParseState) {
                case 0:
                ...
                case 3:
                {
                        ...
                        switch (itemPrefix & (TYPE_MASK | TAG_MASK)) {
							...
							dataIn = OnInputItem(data);				
						//	Serial.println(dataIn[1], HEX);
						//test[var++] = dataIn[var++];
						//Serial.println(dataIn[0], HEX);
...
uint8_t *ReportDescParser2::OnInputItem(uint8_t itm) {
				uint8_t finalResult[100];// = new uint32_t[100];
...
uint32_t* results;
        ...
        for (uint8_t field = 0; field < 62; field++, usage++) {
                ...
                for (uint8_t bits_left = 8, bits_to_copy = 0; bits_left;
				...
				//uint32_t finalResult1[100];
				uint32_t z;
             //   finalResult[z]  =  result.dwResult;
			    finalResult[z]  =  PrintByteValue(result.dwResult);

				z++;
				//finalResult1[0] = PrintByteValue(finalResult[61]);
...
uint8_t UniversalReportParser::Parse(HID *hid, bool is_rpt_id, uint8_t len, uint8_t *buf) {
parsejayam(len,buf);
