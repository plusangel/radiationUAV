...
//friend class ReportDescParser2;
...
public:
        ...
        uint32_t PrintByteValue(uint8_t data);
...
public:
         virtual uint32_t ParseItem(uint8_t **pp, uint16_t *pcntdn);
...
class ReportDescParser : public ReportDescParserBase{
   ...
   uint8_t rptId; // Report ID
      ...
      // Method which is called every time Input item is found
       ...
       // uint8_t *pBufs; // Report buffer pointer
...
//friend class ReportDescParserBase;
...
virtual uint32_t ParseItem(uint8_t **pp, uint16_t *pcntdn);
  ...
  uint8_t *OnInputItem(uint8_t itm); 
...
public:
 /* PASTE THIS WHOLE BLOCK IN TO AFTER public: in the
UniversalReportParser class */
    // ** This is the handle on a parse function. This is done in a
//new way to before.
    // ** This is a function pointer, which should be able to take
    // ** any kind of class/object method or global function (I hope)
    void (*parsejayam)(uint8_t, uint8_t *);

    // ** This is the function where you set the parsing function, similar
    // ** to before, the object just stores the pointer to the function
    void set( void (*func)(uint8_t, uint8_t *) ) {
        parsejayam = func;
    }
  //  void TheExistingParseMethod(uint8_t len, uint8_t *buf) {
//}
        virtual uint8_t Parse(HID *hid, bool is_rpt_id, uint8_t len, uint8_t *buf);
};
